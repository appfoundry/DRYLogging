...
public class LevelAppenderFilter : AppenderFilter {
    /// The level to use when deciding to accept a message or not. The default is LogLevel.trace.
    public let level: LogLevel
    
    /// Flag to indicate if this filter should do an exact match on the level or not. The default is true.
    public let exactMatchRequired: Bool
    
    /// The result returned by decide: when the filter finds matching levels. The default is AppenderFilterDecission.accept.
    public let matchDecission: AppenderFilterDecission
    
    /// The result returned by decide: when the filter finds non-matching levels. The default is AppenderFilterDecission.deny.
    ...
    ///
    /// - parameter level: defaults to trace
    /// - parameter exactMatchRequired: defaults to true
    /// - parameter matchDecission: defaults to accept
    /// - parameter noMatchDecission: defaults to deny

...
- (void)viewDidLoad
{
    ...
    DRYTrace(_logger, @"Controller view did load");
}

- (IBAction)_logATrace:(id)sender {
    DRYTrace(_logger, @"Trace action invoked");
}

- (IBAction)_logADebug:(id)sender {
    DRYDebug(_logger, @"Debug action invoked");
}

- (IBAction)_logAnInfo:(id)sender {
    DRYInfo(_logger, @"Info action invoked");
}

- (IBAction)_logAWarn:(id)sender {
    DRYWarn(_logger, @"Warn action invoked");
}
...
- (IBAction)_logAnError:(id)sender {
    ...
    for (int i = 0; i < 100; i++) {
        DRYError(_logger, @"Error action invoked, let's log a lot so we can show off the rolling file mechanism: %@", loremIpsum);
    }
}

- (void)didReceiveMemoryWarning {
    [super didReceiveMemoryWarning];
    DRYWarn(_logger, @"Memory warning!");
}

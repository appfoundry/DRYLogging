...
class LevelAppenderFilterSpec : QuickSpec {
    override func spec() {
        describe("LevelAppenderFilter") {
            context("loglevel vs filterlevel decision making") {
            ...
            context("default setup") {
                var filter:LevelAppenderFilter!
                
                beforeEach {
                    filter = LevelAppenderFilter()
                }
                
                it("should have trace as filter level") {
                    expect(filter.level) == LogLevel.trace
                }
                
                it("should require an exact match") {
                    expect(filter.exactMatchRequired) == true
                }
                
                it("should have accept as matchDecission") {
                    expect(filter.matchDecission) == AppenderFilterDecission.accept
                }
                
                it("should have deny as noMatchDecission") {
                    expect(filter.noMatchDecission) == AppenderFilterDecission.deny
                }
            }
        }
    }
